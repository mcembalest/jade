...
This installs the current development branch; there is no tagged stable release yet.
...
The [MNIST example](examples/mnist/) and its [Mojo subproject](examples/mnist/mojo/) include short papers alongside their code. The MNIST paper also outlines a jax-js browser-training experiment.
...
- `cmd/jade/`: the launcher.
- `engine/`: Go implementation, tests, and browser UI in `web/`.
- `examples/`: runnable projects and their papers.
- `tests/`: browser tests, the CodeMirror baseline, and fixtures.

`README.md` links to this file so the project overview has one source.
...
Frontend sources live in `engine/web`. Commit the generated files in `engine/web/dist/` after frontend changes so `go install` works without a JavaScript build step. Runtime assets are served locally; dependency licenses are available at `/licenses.txt`.
